 ...
 * @author MSI
 */
public class Admin_Update_Ticket extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Admin_Update_Ticket</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Admin_Update_Ticket at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Ticket t = TicketDB.getTicketById(id);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Ticket newTicket = new Ticket(bill, filmDetail, screenSeat);
        int oldTicketId = Integer.parseInt(request.getParameter("ticketId"));
        Ticket oldTicket = TicketDB.getTicketById(oldTicketId);
        TicketDB.updateTicket(oldTicket, newTicket);
        HttpSession session = request.getSession();
        session.setAttribute("ticketList", TicketDB.getAllTickets());
        request.getRequestDispatcher("full_admin_ticket.jsp").forward(request, response);
    }
