 ...
 * @author MSI
 */
public class Admin_Update_FilmDetail extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Admin_Update_FilmDetail</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Admin_Update_FilmDetail at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        int id = Integer.parseInt(request.getParameter("filmDetailId"));
        FilmDetail f = FilmDetailDB.getFilmDetail(id);
        HttpSession session = request.getSession();
        request.setAttribute("filmDetail", f);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            id = Integer.parseInt(request.getParameter("filmDetailId"));
        ...
        FilmDetailDB.updateFilmDetail(f, newFilmDetail);
        HttpSession session = request.getSession();
        session.setAttribute("filmDetailList", FilmDetailDB.getFilmDetails());
        request.getRequestDispatcher("full_admin_filmDetail.jsp").forward(request, response);
