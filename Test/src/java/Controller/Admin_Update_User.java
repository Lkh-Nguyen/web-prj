 ...
 * @author MSI
 */
public class Admin_Update_User extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Admin_Update_User</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Admin_Update_User at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        int iRole = role(role);
        int oldId = Integer.parseInt(request.getParameter("userId"));
        User oldUser = UserDB.getUser(oldId);
        ...
        if(!email.equals(oldUser.getEmail())&&UserDB.doesUserExist(email)){
        ...
        else if (UserDB.updateUser(oldUser, newUser)) {
        HttpSession session = request.getSession();
        session.setAttribute("userList", UserDB.getAllUsers());
        request.getRequestDispatcher("full_admin_user.jsp").forward(request, response);
